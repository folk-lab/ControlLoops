...
class LS372():
    """ a class for controling the Lakeshore 372 resistance bridge """
    def __init__( self, address, interface='Serial', board = 0):
        rm = visa.ResourceManager()
        if interface == 'GPIB':
            self.ctrl = rm.open_resource('GPIB{0:d}::{1:d}::INSTR'.format(board,address))
            try:
    ...
    def query( self, command_string ):
        """ send the supplied string to the instrument """
        return self.ctrl.query(command_string, delay=0.0)
